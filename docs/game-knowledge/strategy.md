...
| **Guardian kills** | **600** | Free room: 15 guardians × 40 sMoltz each. Kill → drops to region → pick up |
...
Free rooms → earn sMoltz → reach 500 threshold → enter paid rooms (offchain mode).
...
- `rest` gives +1 bonus EP but uses the 30s cooldown — use when EP is low AND no immediate threat.
