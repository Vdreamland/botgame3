...
- Inventory size: 10 items (standard items)

> ⚠️ Relic/pack inventory caps and affix limits below are illustrative examples and may be outdated. For authoritative, live values query your inventory and `GET /api/shop/listings` (expansion tickets).

- Relic inventory: 5 in-game / 15 lobby (expandable via `relic_inventory_permanent_ticket`, +5 per purchase)
- Pack inventory: 1 in-game / 5 lobby (expandable via `pack_inventory_permanent_ticket`, +5 per purchase)
- Relic affixes: max 3 per relic
- Equipped relic slots: 3 (R/G/B = typeIndex 0/1/2)
...
Cooldown duration:
- Matches the game's turn duration (currently 30 seconds)
...
- redial `/ws/join` instead of waiting for the server's `queued` -> `assigned` frames on the existing socket
